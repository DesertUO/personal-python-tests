...
def generar_entradas_notas():
    ...
    if seleccion == "Promedio Laboratorio":
        ...
        tk.Label(frame_dynamic, text="Notas PC:").grid(row=0, column=0)
        for i in range(cantidad_pc):
            tk.Entry(frame_dynamic, textvariable=var_notas_pc[i]).grid(row=i + 1, column=0)

        tk.Label(frame_dynamic, text="Notas TI:").grid(row=0, column=1)
        for i in range(cantidad_ti):
            tk.Entry(frame_dynamic, textvariable=var_notas_ti[i]).grid(row=i + 1, column=1)

    elif seleccion == "Cobrar Cuota":
        cantidad = var_cantidad_companeros.get()
        # Limpiar las notas de los compañeros
        global var_notas_companeros
        var_notas_companeros = [tk.IntVar() for _ in range(cantidad)]
        
        tk.Label(frame_dynamic, text="Notas:").grid(row=0, column=0)
        for i in range(cantidad):
            tk.Entry(frame_dynamic, textvariable=var_notas_companeros[i]).grid(row=i + 1, column=0)
